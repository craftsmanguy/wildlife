...
@Stateless(name = "AdvertAdministrationManager")
...
public class AdvertAdministrationManager implements AdvertAdministrationLocal {
	...
	public AbstractFormatDto saveFormat(AbstractFormatDto format)
		...
		AbstractFormatDto result = null;

		throwsExceptionIfAllFieldsAreNotFill(format);
		sanitizeFieldsOfFormat(format);
		throwsExceptionIfFormatEnumNotContainValue(((FormatAdministratorDto) format).getName());
		((FormatAdministratorDto) format).setActive(false);
	...
	public Set<AbstractFormatDto> searchFormats(AbstractFormatDto format) throws EntityNotFoundException {
		Set<AbstractFormatDto> results = advertFacade.searchFormats(format);
		if (results.isEmpty()) {
	...
	private void sanitizeFieldsOfFormat(AbstractFormatDto format) {
		((FormatAdministratorDto) format).setName(TransformationHelper.cleanAllSpecialsCharacters(((FormatAdministratorDto) format).getName().toUpperCase()));
	...
	private void throwsExceptionIfAllFieldsAreNotFill(AbstractFormatDto format) throws RequiredFieldException {
		if (format == null || StringUtils.isEmpty(((FormatAdministratorDto) format).getName()) || StringUtils.isEmpty(format.getFeature())) {
			throw new RequiredFieldException(BAD_REQUEST, ErrorEntity.ErrorKey.FIELD_IS_MISSING.getValue());
		}
	}

	private void getCodeFromFields(AbstractFormatDto format) throws RestClientException {
		String textToEncrypt = ((FormatAdministratorDto) format).getName() + "-" + format.getFeature();
